...
class Worker:
    ...
    def __init__(self, ipfs_path, num_workers, idx, topk, key, is_evil):
        self.bcc = BCCommunicator()
        self.fsc = FSCommunicator(ipfs_path)
        

        model  = self.fsc.fetch_initial_model()
        ...
        # print("opt",opt)
        self.model = Model(num_workers, idx, model, topk, is_evil)
